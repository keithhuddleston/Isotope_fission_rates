...
#Contains the half lives of varius isotopes.
hl = {
...
#Contains the atomic numbers of the respective isotopes (used to convert between mass and number)    
molwe = {
...
#Contains the cross sections of varius isotopes when incident neutrons are at 0.0253 ev.
xs = {
...
#Tranmutation Modes    
tm = {
    "Th_231": {"decay": "Pa_231", "capture": "Th_232"}, 
...
def number(isotope, mass_0, flux, time):
    """
    Given mass of a pure isotope, calculates the number of atoms of all isotopes
    over time.
    """
    
    #Column vector containing original number.
    no = [0]*len(mo)
    ...
    #Creates a nxn matrix where n is the number of considered isotopes.    
    M = np.zeros((len(mo),len(mo)))
    ...
    #Solves the system of differential equations housed within the matrix.
    def y_prime(y, time, M):
        return(np.dot(M, y))
...
def fission_rate(isotope, mass_0 = 1E-6, flux = 1E13, endtime = 3.1536E7):
    ...
    #Converts the values in the matrix from number of atoms to fission rates.
    for i in range(0, len(time)): 
        for j in range(0,len(sol[0])):
            sol[i][j] = sol[i][j]*flux*xs[mo[j]]["fission"]*10**-24
            
    #Creates a column vector containing the total fission rate at each time interval.       
    fission = []
    fissionsum = []  
    for i in range(0,len(time)):
        for j in range(0,len(sol[0])):
    ...
    #Sums all isotope contributions that do not meet the 5% condition.
    other = []     
    for i in range(0,len(time)):
        temp = [] 
        for j in range(0,len(sol[0])):
    ...
    #Creates a list containing names that will be displayed in the legend.    
    legend = []   
    for i in sf:  
        legend.append(mo[i])
    legend.append("Other")
...
def mass(isotope, mass_0 = 1E-6, flux = 1E13, endtime = 3.1536E7):
    """
    Calculates the mass (in micrograms) of each isotope for given initial 
    ...
    #Sufficient mass condition, 0.01 microgram and below discriminated out of plot.
    sm = []
    for i in range(0,len(time)):
        for j in range(0,len(sol[0])):
            if sol[i][j] > 0.01:
